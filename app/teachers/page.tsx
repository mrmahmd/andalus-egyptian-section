...
export default function TeachersDashboardPage() {
  ...
  const saveWholeWeek = async (submitForReview = false, silent = false) => {
    ...
    try {
      ...
      if (!silent) {
        setMessage("Your plan was saved. It will publish automatically when every assigned teacher completes the required lessons for this class and week.");
    ...
    <main className="teacher-portal">
        ...
        <div className="teacher-content">
          ...
          {(activeNav === "Overview" || activeNav === "Weekly Plans") && <>
            ...
            <section className="teacher-card teacher-review-status-card" hidden>
              <div className="teacher-card-heading"><div><p className="teacher-kicker">Plan follow-up</p><h2>My weekly plans</h2><p>Your work stays editable. The class plan becomes visible to families automatically after every assigned teacher completes their timetable lessons.</p></div></div>
      ...
      {weeklyBuilderOpen && selectedClass && selectedWeek && <div className="teacher-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && !saving && setWeeklyBuilderOpen(false)}><section className="teacher-editor-modal weekly-builder-modal" role="dialog" aria-modal="true" aria-labelledby="weekly-builder-title">
        ...
        <form onSubmit={(event) => { event.preventDefault(); void saveWholeWeek(false); }}>
          ...
          <div className="teacher-editor-footer"><span>{selectedClassSlots.length > 0 ? "Every card is one of your real timetable lessons. Save your work and it will publish automatically when the class week is complete." : "Saving is blocked until the timetable is connected."}</span><div><button disabled={saving || selectedClassSlots.length === 0} type="button" className="teacher-secondary-button teacher-preview-button" onClick={() => void openParentPreview()}>Preview parent plan</button><button disabled={saving} type="button" className="teacher-secondary-button" onClick={() => setWeeklyBuilderOpen(false)}>Cancel</button><button disabled={saving || selectedClassSlots.length === 0} type="button" className="teacher-secondary-button" onClick={() => void saveWholeWeek(false)}>Save draft</button><button disabled={saving || selectedClassSlots.length === 0} className="teacher-primary-button" type="submit">{saving ? "Saving…" : "Save & publish automatically"}</button></div></div>
        </form>
      </section></div>}
