...
export default function WeeklyPlanPage() {
  ...
  return <main className="subpage plan-page">
    ...
    <section id="selected-plan" className={`selected-plan-wrap ${selectedPlan ? "is-open" : ""}`} aria-hidden={!selectedPlan}>{selectedPlan && <><div className="plan-toolbar page-width"><div><p className="eyebrow">OFFICIAL WEEKLY PLAN</p><h1>{grade} · {classType}</h1><p>Week {selectedPlan.weekNumber} · {selectedPlan.weekLabel || formatDates(selectedPlan.startsOn, selectedPlan.endsOn)}</p></div><div className="toolbar-actions"><button className="button button-outline" type="button" onClick={() => setSelectedWeek(null)}>Back to Plans</button><button className="button button-primary" type="button" onClick={() => window.print()}>Print / Save PDF</button></div></div>
      <section className="plan-paper page-width" aria-label="Weekly study plan"><div className="paper-header"><img src={`${basePath}/school-logo.jpeg`} alt="AlAndalus Private Schools" /><div><strong>ALANDALUS PRIVATE SCHOOLS</strong><span>The Egyptian Section</span><h2>WEEKLY STUDY PLAN</h2></div></div><div className="paper-meta"><span><small>Class</small><strong>{grade} · {classType}</strong></span><span><small>Class Teacher</small><strong>To be confirmed</strong></span><span><small>Week No.</small><strong>{selectedPlan.weekNumber}</strong></span><span><small>Date</small><strong>{selectedPlan.weekLabel || formatDates(selectedPlan.startsOn, selectedPlan.endsOn)}</strong></span></div>{liveDictations.map((dictation, index) => <section className="parent-dictation-block" key={`${dictation.day}-${index}`}><h3>Vocabulary for Dictation on {dayNames[dictation.day]}</h3><table><tbody>{chunkWords(dictation.words).map((row, rowIndex) => <tr key={rowIndex}>{row.map((word) => <td key={word}>{word}</td>)}</tr>)}</tbody></table></section>)}<div className="table-wrap"><table className="weekly-table"><thead><tr><th>Day</th><th>Course</th><th>Classwork</th><th>Homework</th><th>Classera Notes</th></tr></thead>{days.map(({ day, lessons }) => <tbody className="weekly-day-group" key={day}>{lessons.map((lesson, lessonIndex) => <tr key={`${day}-${lesson.period_number}`} className={lessonIndex === 0 ? "new-day" : ""}>{lessonIndex === 0 && <td className="day-cell" rowSpan={lessons.length}>{day}</td>}<td className="course-cell">{lesson.course}</td><td>{lesson.classwork || "—"}</td><td>{lesson.homework || "—"}</td><td>{lesson.notes || "—"}</td></tr>)}</tbody>)}</table>{days.length === 0 && <p className="supervisor-review-empty">This approved plan does not contain lesson entries yet.</p>}</div>{liveQuizzes.length > 0 && <div className="important-notes"><strong>Quizzes & Assessments</strong><div className="parent-plan-assessments">{liveQuizzes.map((quiz, index) => <p key={`${quiz.course}-${index}`}><strong>{quiz.course}{quiz.date ? ` · ${quiz.date}` : ""}</strong>{quiz.details}</p>)}</div></div>}</section></>}</section>
  </main>;
}
